        ...
        <div class="content-area">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="modern-table">
                                <thead><tr><th>ID</th><th>Client</th><th>Purpose</th><th>Certificates</th><th>Total</th><th>Control No</th><th>Status</th><th>Action</th><th>Date</th></tr></thead>
                                <tbody>
                                <?php
                                $sql = "SELECT r.id, CONCAT(c.firstname,' ',c.middlename,' ',c.lastname) AS fullname, c.purpose, r.total_amount, r.control_number, r.status, r.created_at FROM requests r JOIN clients c ON r.client_id = c.id ORDER BY r.created_at DESC";
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if($row['status']=='PAID'): ?>
                                            <a href="home.php?prepare=<?= $row['id'];?>" class="modern-btn modern-btn-info modern-btn-sm"><i class="fas fa-check-circle me-1"></i> Prepared</a>
                                        <?php elseif($row['status']=='PREPARED'): ?>
                                            <a href="home.php?release=<?= $row['id'];?>" class="modern-btn modern-btn-primary modern-btn-sm"><i class="fas fa-check-double me-1"></i> Released</a>
                                        <?php else: ?>
            ...
            <?php elseif($tab=='history'): ?>
                ...
                <div class="modern-card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="modern-table">
                                <thead><tr><th>ID</th><th>Client</th><th>Purpose</th><th>Certificates</th><th>Total</th><th>Control No</th><th>Status</th><th>Date</th></tr></thead>
                                <tbody>
                                <?php
                ...
                <div class="modern-card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="import_logic.php" method="POST" enctype="multipart/form-data">
                            ...
                            <button type="submit" name="import" class="modern-btn modern-btn-success"><i class="fas fa-upload me-2"></i> Import Data</button>
                        </form>
                    </div>
                </div>
            ...
            <?php elseif($tab=='find'): ?>
                ...
                <?php if(!empty($results)): ?>
                    <div class="modern-card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-primary" style="font-size: 0.9rem; padding: 8px 15px; border-radius: 30px;"><i class="fas fa-database me-1"></i> Found: <?= count($results) ?> record(s)</span>
                        </div>
                        <div class="card-body p-0">
                            <div class="scrollable-container">
                                <div class="table-responsive">
                                    <table class="modern-table find-record-table">
                                        <thead><tr><th>Declared Owner</th><th>Owner Address</th><th>Property Location</th><th>Title</th><th>Lot</th><th>ARP No.</th><th>PIN No.</th><th>Classification</th><th>Actual Use</th><th>Area</th><th>Market Value</th><th>Assessed Value</th><th>Taxability</th><th>Effectivity</th><th>Cancellation</th></tr></thead>
                                        <tbody>
                ...
                <?php elseif(!empty($search_name)): ?>
                    <div class="modern-alert modern-alert-warning"><i class="fas fa-exclamation-triangle me-2"></i> No records found for "<?= htmlspecialchars($search_name) ?>" in <?= ucwords(str_replace('_',' ',$selected_barangay)) ?>.</div>
                <?php endif; ?>
            ...
            <?php elseif($tab=='faas'): ?>
                ...
                $fields = ['declared_owner','owner_address','property_location','title','lot','ARP_No.','PIN_No.','classification','actual_use','area','mv','av','taxability','effectivity','cancellation'];
                $labels = ['Declared Owner','Owner Address','Property Location','Title','Lot','ARP No.','PIN No.','Classification','Actual Use','Area','Market Value','Assessed Value','Taxability','Effectivity','Cancellation'];
                ...
                if(isset($_POST['save_faas'])){
                    ...
                    if($_POST['mode']=='add'){
                        $stmt = $conn->prepare("INSERT INTO `$brgy` (declared_owner,owner_address,property_location,title,lot,`ARP_No.`,`PIN_No.`,classification,actual_use,area,mv,av,taxability,effectivity,cancellation) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                        $stmt->bind_param("sssssssssssssss", $data['declared_owner'],$data['owner_address'],$data['property_location'],$data['title'],$data['lot'],$data['ARP_No.'],$data['PIN_No.'],$data['classification'],$data['actual_use'],$data['area'],$data['mv'],$data['av'],$data['taxability'],$data['effectivity'],$data['cancellation']);
                    } elseif($_POST['mode']=='edit'){
                        $original_pin = $_POST['original_pin'];
                        $stmt = $conn->prepare("UPDATE `$brgy` SET declared_owner=?,owner_address=?,property_location=?,title=?,lot=?,`ARP_No.`=?,`PIN_No.`=?,classification=?,actual_use=?,area=?,mv=?,av=?,taxability=?,effectivity=?,cancellation=? WHERE `PIN_No.`=?");
                        $stmt->bind_param("ssssssssssssssss", $data['declared_owner'],$data['owner_address'],$data['property_location'],$data['title'],$data['lot'],$data['ARP_No.'],$data['PIN_No.'],$data['classification'],$data['actual_use'],$data['area'],$data['mv'],$data['av'],$data['taxability'],$data['effectivity'],$data['cancellation'],$original_pin);
                    }
                    $stmt->execute();
                    $stmt->close();
                    ...
                    <div class="card-body">
                        <form method="GET" class="row g-3">
                            ...
                            <?php if($selected_barangay): ?>
                                <div class="col-md-3">
                                    <a href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&action=add" class="modern-btn modern-btn-success w-100"><i class="fas fa-plus-circle me-2"></i> Add FAAS</a>
                                </div>
                            <?php endif; ?>
                        </form>
                    </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="row g-3">
                                <?php foreach($fields as $i=>$field): ?>
                                    <div class="col-md-6">
                                        <input name="<?= $field ?>" class="modern-form-control" placeholder="<?= $labels[$i] ?>" value="<?= htmlspecialchars($edit_row[$field] ?? '') ?>" required>
                                    </div>
                                <?php endforeach; ?>
                                <div class="col-md-12 mt-4">
                                    ...
                                    <a href="home.php?tab=faas&barangay=<?= $selected_barangay ?>" class="modern-btn modern-btn-secondary ms-2"><i class="fas fa-times me-2"></i> Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div class="modern-card">
                    <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                        <span class="mb-2 mb-md-0"><i class="fas fa-table me-2"></i> FAAS Records - <?= ucwords(str_replace('_',' ',$selected_barangay)) ?></span>
                        <div class="d-flex flex-wrap gap-2">
                            <span class="badge bg-primary" style="font-size: 0.9rem; padding: 8px 15px; border-radius: 30px;"><i class="fas fa-database me-1"></i> Total: <?= number_format($total_records) ?> records</span>
                            <a href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&action=add" class="modern-btn modern-btn-success modern-btn-sm"><i class="fas fa-plus-circle me-1"></i> Add New</a>
                        </div>
                    </div>
                    <div class="card-body p-0">
                        <div class="scrollable-container">
                            <div class="table-responsive">
                                <table class="modern-table faas-table">
                                    <thead><tr><?php foreach($labels as $label): ?><th><?= $label ?></th><?php endforeach; ?><th class="text-center">Actions</th></tr></thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    <a href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&action=edit&id=<?= urlencode($row['PIN_No.']) ?>&page=<?= $page ?>" class="modern-btn modern-btn-warning modern-btn-sm" title="Edit"><i class="fas fa-edit"></i> <span class="d-none d-md-inline ms-1">Edit</span></a>
                                                    <a href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&action=delete&id=<?= urlencode($row['PIN_No.']) ?>&page=<?= $page ?>" class="modern-btn modern-btn-danger modern-btn-sm" title="Delete" onclick="return confirm('Delete this record?');"><i class="fas fa-trash"></i> <span class="d-none d-md-inline ms-1">Delete</span></a>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                    ...
                    <div class="card-footer bg-transparent">
                        <nav aria-label="Page navigation">
                            <ul class="pagination mb-0 justify-content-center flex-wrap" style="gap: 3px;">
                                <?php if($page > 1): ?>
                                    <li class="page-item"><a class="page-link" href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&page=1"><i class="fas fa-angle-double-left"></i></a></li>
                                    <li class="page-item"><a class="page-link" href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&page=<?= $page-1 ?>"><i class="fas fa-chevron-left"></i></a></li>
                                <?php endif; ?>
                                <?php for($i = max(1, $page-2); $i <= min($total_pages, $page+2); $i++): ?>
                                    <li class="page-item <?= $i==$page ? 'active' : '' ?>"><a class="page-link" href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&page=<?= $i ?>"><?= $i ?></a></li>
                                <?php endfor; ?>
                                <?php if($page < $total_pages): ?>
                                    <li class="page-item"><a class="page-link" href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&page=<?= $page+1 ?>"><i class="fas fa-chevron-right"></i></a></li>
                                    <li class="page-item"><a class="page-link" href="home.php?tab=faas&barangay=<?= $selected_barangay ?>&page=<?= $total_pages ?>"><i class="fas fa-angle-double-right"></i></a></li>
                                <?php endif; ?>
                            </ul>
                        </nav>
                    </div>
            ...
            <div class="modern-card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="modern-table">
                            <thead><tr><th>ID</th><th>Certificate Name</th><th>Description</th><th>Price</th><th>Status</th><th>Created At</th><th>Actions</th></tr></thead>
                            <tbody>
                            <?php if($certificates->num_rows > 0): ?>
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="7" class="text-center py-4"><div class="empty-state"><i class="fas fa-certificate"></i><p>No certificates found. Click "Add New Certificate" to create one.</p></div></td></tr>
                            <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
