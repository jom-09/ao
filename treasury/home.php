...
================================= */
$dash_paid_services_count = 0;
$dash_paid_tax_count      = 0;
$dash_paid_cert_count     = 0;
$top_barangay_labels = [];
$top_barangay_values = [];

if ($view === 'dashboard') {
    // 1) Count PAID Services
    try {
...
// Counts for badges in sidebar/requests
...
if ($view === 'requests') {
    ...
    try {
        $taxPendingCount = (int)$conn->query("
            SELECT COUNT(*) c
            FROM tax_requests
...
<head>
    ...
    <link href="../assets/bootstrap/css/tstyle.css" rel="stylesheet">
</head>
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="sidebar-menu">
            <a href="home.php?view=dashboard" class="sidebar-link <?= $view==='dashboard' ? 'active' : '' ?>">
                <i class="fas fa-chart-pie"></i> Dashboard
            </a>
            <a href="home.php?view=requests" class="sidebar-link <?= $view==='requests' ? 'active' : '' ?>">
                <i class="fas fa-inbox"></i> Requests
                <?php if(($pendingCertSvcCount + $taxClearanceCount + $taxPendingCount) > 0): ?>
                    <span class="badge bg-danger rounded-pill ms-auto"><?= $pendingCertSvcCount + $taxClearanceCount + $taxPendingCount ?></span>
                <?php endif; ?>
            </a>
            <a href="home.php?view=tax_history" class="sidebar-link <?= $view==='tax_history' ? 'active' : '' ?>">
                <i class="fas fa-receipt"></i> Tax History
            </a>
            <a href="home.php?view=installments" class="sidebar-link <?= $view==='installments' ? 'active' : '' ?>">
                <i class="fas fa-calendar-check"></i> Installments
            </a>
            <a href="home.php?view=transactions" class="sidebar-link <?= $view==='transactions' ? 'active' : '' ?>">
                <i class="fas fa-history"></i> Transactions
            </a>
            <div class="mt-4 mb-2 px-3 text-uppercase text-muted" style="font-size: 0.7rem; font-weight: 700;">Archive</div>
            <a href="search_archive.php" class="sidebar-link">
                <i class="fas fa-search"></i> Search Archive
            </a>
            <a href="import_archive.php" class="sidebar-link">
                <i class="fas fa-file-import"></i> Import Archive
            </a>
        </div>
    ...
    <div class="main-content">
        ...
        <header class="topbar">
            <div class="d-flex align-items-center">
                ...
                <h5 class="m-0 fw-bold d-none d-sm-block">
                    <?= ucfirst(str_replace('_', ' ', $view)) ?>
                </h5>
            </div>
            <div class="user-profile">
                <button type="button" class="icon-btn" id="openTreasuryMessageModalBtn" data-bs-toggle="modal" data-bs-target="#treasuryMessageModal" title="Messages">
                    <i class="fas fa-envelope"></i>
                    <span id="treasuryUnreadBadge" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="display:none; font-size: 0.6rem;">0</span>
                </button>
                <div class="user-info">
                    <p class="user-name"><?php echo h($_SESSION['fullname'] ?? 'Treasury Officer'); ?></p>
                    <p class="user-role">Treasury Department</p>
                </div>
                <div class="dropdown">
        ...
        <div class="p-4">
            ...
            <?php if ($view === 'dashboard'): ?>
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-4">
                        <div class="stat-card">
                            <div>
                                <p class="text-muted mb-1 text-uppercase fw-bold" style="font-size: 0.75rem;">Paid Services</p>
                                <h3 class="fw-bold mb-0"><?= (int)$dash_paid_services_count ?></h3>
                            </div>
                            <div class="stat-icon blue"><i class="fas fa-handshake-angle"></i></div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="stat-card">
                            <div>
                                <p class="text-muted mb-1 text-uppercase fw-bold" style="font-size: 0.75rem;">Certificates Released</p>
                                <h3 class="fw-bold mb-0"><?= (int)$dash_paid_cert_count ?></h3>
                            </div>
                    ...
                    <div class="col-md-4">
                        <div class="stat-card">
                            <div>
                                <p class="text-muted mb-1 text-uppercase fw-bold" style="font-size: 0.75rem;">Paid Tax</p>
                                <h3 class="fw-bold mb-0"><?= (int)$dash_paid_tax_count ?></h3>
                            </div>
                ...
                <div class="dashboard-card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="m-0">Top Barangay (Tax Paid Count)</h5>
                        </div>
                    </div>
                    <div class="card-body">
            ...
            <?php if ($view === 'requests'): ?>
                ...
                <div class="search-section">
                    <div class="d-flex align-items-center mb-3">
                        ...
                        <h3 class="h5 fw-bold m-0">Search Person & Records</h3>
                    </div>
                    <div class="row g-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-7">
                            <div class="search-input-wrapper">
                                ...
                                <input type="text" class="form-control form-control-lg person-search" id="personSearch" placeholder="Enter owner name...">
                            </div>
                        </div>
                        <div class="col-md-2">
                            <button class="btn search-btn w-100" id="searchPersonBtn">
                ...
                <div class="records-section dashboard-card" id="recordsSection" style="display:none;">
                    <div class="card-header">
                        <h5 class="m-0"><i class="fas fa-database me-2"></i>Property Records</h5>
                        <span class="badge bg-light text-dark" id="recordCount"></span>
                    </div>
                ...
                <div class="dashboard-card">
                    <div class="card-header">
                        ...
                        <span class="badge bg-warning text-dark" id="pendingCount"><?php echo (int)$pendingCertSvcCount; ?></span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="modern-table" id="pendingTable">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            <td><span class="badge bg-light text-dark border">#<?php echo (int)$row['id']; ?></span></td>
                                            ...
                                            <td><small class="text-muted"><?php echo h($items); ?></small></td>
                                            <td class="text-end fw-bold">₱<?php echo number_format((float)$row['total_amount'],2); ?></td>
                                            <td><span class="date"><?php echo date('M d, Y', strtotime($row['created_at'])); ?></span></td>
                                            <td class="text-end">
                                                <a href="process_request.php?id=<?php echo (int)$row['id'];?>" class="action-btn accept" title="Accept"><i class="fas fa-check"></i></a>
                                                <a href="process_request.php?decline=<?php echo (int)$row['id'];?>" class="action-btn decline" title="Decline"><i class="fas fa-times"></i></a>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
                ...
                <div class="dashboard-card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="m-0">Tax Clearance Requests</h5>
                        </div>
                        <span class="badge bg-info text-dark" id="taxClearanceCount"><?php echo (int)$taxClearanceCount; ?></span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="modern-table" id="taxClearanceTable">
                                ...
                                <tbody>
                                    ...
                                    try {
                                        $sqlTC = "
                                            SELECT c.id, c.firstname, c.middlename, c.lastname, c.address, c.cp_no, c.purpose, c.created_at
                                            FROM clients c
                                            WHERE c.purpose='Tax Clearance'
                                                AND NOT EXISTS (
                                                    SELECT 1 FROM requests r WHERE r.client_id = c.id AND r.status = 'PAID'
                                                )
                                            ORDER BY c.created_at DESC
                                        ";
                                        $resultTC = $conn->query($sqlTC);
                                        while($rowTC = $resultTC->fetch_assoc()):
                                            $fullname = trim(($rowTC['firstname'] ?? '') . ' ' . ($rowTC['middlename'] ?? '') . ' ' . ($rowTC['lastname'] ?? ''));
                                    ?>
                                            <tr>
                                                <td><span class="badge bg-light text-dark border">#<?php echo (int)$rowTC['id']; ?></span></td>
                                                ...
                                                <td class="text-end">
                                                    <button type="button" class="action-btn info btnDoneTaxClearance" data-id="<?php echo (int)$rowTC['id']; ?>" title="Mark Done"><i class="fas fa-check"></i></button>
                                                </td>
                                            </tr>
                                    <?php
                                        endwhile;
                ...
                <div class="dashboard-card">
                    <div class="card-header">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="m-0">Tax Payment Requests</h5>
                        </div>
                        <span class="badge bg-success" id="taxPendingCount"><?php echo (int)$taxPendingCount; ?></span>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="modern-table" id="taxPendingTable">
                                ...
                                <tbody>
                                    <?php
                                    try {
                                        $taxSql = "SELECT id, declared_owner, arp_no, assessed_value, created_at, status FROM tax_requests WHERE status='PENDING' ORDER BY created_at DESC";
                                    ...
                                    ?>
                                            <tr>
                                                <td><span class="badge bg-light text-dark border">#<?php echo (int)$t['id']; ?></span></td>
                                                <td><?php echo h($t['declared_owner']); ?></td>
                                                <td><?php echo h($t['arp_no']); ?></td>
                                                <td class="text-end">₱<?php echo number_format($av, 2); ?></td>
                                                <td class="text-end fw-bold">₱<?php echo number_format($base, 2); ?></td>
                                                ...
                                                <td class="text-end">
                                                    <button type="button" class="action-btn accept btnProcessTax" title="Process Payment"
            ...
            <div class="dashboard-card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="m-0">
                        <i class="fas fa-receipt me-2"></i>Tax Payment History
                    </h5>

                    <button type="button" class="btn btn-success btn-sm" onclick="exportPaidTaxesExcel()">
                        <i class="fas fa-file-excel me-1"></i> Export Paid Taxes
                    </button>
                </div>
                            ...
                            <tbody>
                                ...
                                try {
                                    ...
                                    while ($t = $histRes->fetch_assoc()):
                                        $av = (float)str_replace([',',' '], '', ($t['assessed_value'] ?? '0'));
                                        $base = $av * 0.02;
                                ...
                                ?>
                                    <tr
                                        ...
                                        data-owner="<?php echo h($t['declared_owner']); ?>"
                                        data-arp="<?php echo h($t['arp_no']); ?>"
                                        data-tax="<?php echo number_format($base, 2, '.', ''); ?>"
                                        data-created="<?php echo h($t['created_at']); ?>"
                                    >
                                        <td>#<?php echo (int)$t['id']; ?></td>
                                        <td><?php echo h($t['declared_owner']); ?></td>
                                        <td><?php echo h($t['arp_no']); ?></td>
                                        <td class="text-end">₱<?php echo number_format($base, 2); ?></td>
                                        <td><?php echo h($t['control_number'] ?? ''); ?></td>
                                        <td>
                                            <?php if ($status === 'PAID'): ?>
                                                <span class="status-badge paid">PAID</span>
                                            <?php elseif ($status === 'INSTALLMENT'): ?>
                                                <span class="status-badge pending">INSTALLMENT</span>
                                            <?php else: ?>
                                                <span class="status-badge declined">DECLINED</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?php echo date('M d, Y', strtotime($dateShow)); ?></td>
                                    </tr>
                                <?php
                                    endwhile;
                                } catch (Throwable $e) { }
            ...
            <script>
                async function exportPaidTaxesExcel() {
                    const rows = document.querySelectorAll('#taxHistoryTable tbody tr');
                    const paidRows = [];
                    ...
                    if (paidRows.length === 0) {
                        alert('No PAID tax records to export.');
                        return;
                    }

                    const workbook = new ExcelJS.Workbook();
                    const worksheet = workbook.addWorksheet('Paid Taxes');

                    worksheet.columns = [
                        { header: 'Owner', key: 'owner', width: 35 },
                        { header: 'ARP No.', key: 'arp', width: 25 },
                    ...
                    saveAs(blob, 'paid_tax_report.xlsx');
                }
            </script>
        <?php endif; ?>
            ...
            <?php if ($view === 'installments'): ?>
                <div class="dashboard-card">
                    <div class="card-header">
                        <h5 class="m-0"><i class="fas fa-calendar-check me-2"></i>Installment Payments</h5>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="modern-table" id="installmentsTable">
                                ...
                                <tbody>
                                    <?php
                                    try {
                                        $instSql = "SELECT id, declared_owner, arp_no, control_number, COALESCE(paid_at, created_at) AS started_at FROM tax_requests WHERE status='INSTALLMENT' ORDER BY COALESCE(paid_at, created_at) DESC";
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td><?php echo h($t['arp_no']); ?></td>
                                                <td><?php echo h($t['control_number'] ?? ''); ?></td>
                                                <td><?php echo $t['started_at'] ? date('M d, Y', strtotime($t['started_at'])) : ''; ?></td>
                                                <td class="text-end">
                                                    <button type="button" class="action-btn info btnViewInstallment" title="View Schedule"
                                                        data-id="<?php echo (int)$t['id']; ?>"
                                                        data-owner="<?php echo h($t['declared_owner']); ?>"
                                                        data-arp="<?php echo h($t['arp_no']); ?>"
            ...
            <?php if ($view === 'transactions'): ?>
                <div class="dashboard-card">
                    <div class="card-header">
                        <h5 class="m-0"><i class="fas fa-history me-2"></i>Transaction History</h5>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="modern-table" id="historyTable">
                                ...
                                <tbody>
                                    <?php
                                    $sql2 = "
                                        SELECT r.id, CONCAT(c.firstname,' ',c.middlename,' ',c.lastname) AS fullname, c.purpose, r.total_amount, r.control_number, r.status, r.paid_at, r.created_at
                                        FROM requests r
                                        JOIN clients c ON r.client_id = c.id
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><?php echo h($row['fullname']); ?></td>
                                            <td><?php echo h($row['purpose']); ?></td>
                                            <td class="fw-bold">₱<?php echo number_format((float)$row['total_amount'],2); ?></td>
                                            <td><?php echo h($row['control_number']); ?></td>
                                            <td>
                                            ...
                                            <td><?php echo date('M d, Y', strtotime($row['paid_at'] ? $row['paid_at'] : $row['created_at'])); ?></td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
                            </table>
...
<script>
    ...
    <script>
    // Sidebar Toggle (New Design Specific)
    ...
    $(document).ready(function() {
        ...
        function buildTaxClearanceRow(r){
            ...
            return [
                ...
                `
                    <button
                        type="button"
                        class="action-btn info btnDoneTaxClearance"
                        title="Mark as Done"
                        data-id="${id}">
        ...
        if (isRequestsView) {
            setTimeout(function(){
                ...
                const pFirst = $('#pendingTable tbody tr:first .badge').text().replace('#','');
                const tcFirst = $('#taxClearanceTable tbody tr:first .badge').text().replace('#','');
                const tFirst = $('#taxPendingTable tbody tr:first .badge').text().replace('#','');
